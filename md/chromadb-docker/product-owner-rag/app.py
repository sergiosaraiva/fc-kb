...
4. Brief Prophix.Conso mention (1-2 sentences)
""",
        "Standard": """
...
- Prophix.Conso financial consolidation software implementation
...
Provide detailed step-by-step instructions for using the feature in Prophix.Conso:
...
3. Describes Prophix.Conso implementation
...
def get_fallback_follow_ups(query: str) -> list:
    ...
    if "equity" in query_lower:
        ...
        return [
            ...
            "How does Prophix.Conso implement this?"
        ]
...
def main():
    ...
    # Check for concept map query from sessionStorage (via JavaScript)
    # This runs early to capture queries from concept map clicks
    ...
    """
    st.components.v1.html(concept_map_check_js, height=0)

    # Check URL params for concept map query
    query_params = st.query_params
    if "cm_query" in query_params:
        cm_query = query_params["cm_query"]
        # Clear the URL param
...
"""
    ...
    if st.session_state.get("show_concept_map"):
        ...
        # vis.js concept map - increased height and consistent styling
        concept_map_html = """
        <style>
            #concept-map-legend, #selected-concept {
                font-family: "Source Sans Pro", sans-serif;
                font-size: 14px;
            }
        </style>
        <div id="concept-map-container" style="width: 100%; height: 650px; border: 1px solid #30363D; border-radius: 8px; background-color: #0D1117;"></div>
        <div id="concept-map-legend" style="margin-top: 10px; display: flex; gap: 20px; flex-wrap: wrap; color: #C9D1D9;">
            <span><span style="display:inline-block;width:12px;height:12px;background:#5B7FFF;border-radius:50%;margin-right:4px;"></span>Methods</span>
            <span><span style="display:inline-block;width:12px;height:12px;background:#2EA043;border-radius:50%;margin-right:4px;"></span>Standards</span>
            <span><span style="display:inline-block;width:12px;height:12px;background:#A371F7;border-radius:50%;margin-right:4px;"></span>Calculations</span>
            <span><span style="display:inline-block;width:12px;height:12px;background:#F0883E;border-radius:50%;margin-right:4px;"></span>Eliminations</span>
            <span><span style="display:inline-block;width:12px;height:12px;background:#58A6FF;border-radius:50%;margin-right:4px;"></span>Currency</span>
            <span><span style="display:inline-block;width:12px;height:12px;background:#DB6D6D;border-radius:50%;margin-right:4px;"></span>Concepts</span>
        </div>
        <div id="selected-concept" style="margin-top: 10px; padding: 10px; background: rgba(91,127,255,0.1); border-radius: 4px; display: none; color: #C9D1D9;">
            <strong>Selected:</strong> <span id="concept-name"></span>
            <br><button id="search-concept-btn" onclick="searchSelectedConcept()" style="margin-top:8px; width: 100%; padding: 10px 16px; border: none; border-radius: 4px; background: #5B7FFF; color: white; font-size: 14px; cursor: pointer; font-weight: 500;">
                Search: <span id="concept-query-text"></span>
            </button>
        </div>
        ...
        function searchSelectedConcept() {
            if (selectedQuery) {
                // Store query in sessionStorage and reload parent page
                window.parent.sessionStorage.setItem('concept_map_query', selectedQuery);
                window.parent.location.reload();
            }
        }

        // Click handler - show search button
        ...
        """
        st.components.v1.html(concept_map_html, height=800)

        # JavaScript listener for concept map clicks
        # We'll use a text input hidden approach to capture the query
        concept_search_js = """
        <script>
        window.addEventListener('message', function(event) {
            if (event.data && event.data.type === 'concept_map_search') {
                // Create a custom event or use sessionStorage
                sessionStorage.setItem('concept_map_query', event.data.query);
                // Trigger a reload to pick up the query
                window.location.reload();
            }
        });
        // Check for stored query on load
        const storedQuery = sessionStorage.getItem('concept_map_query');
        if (storedQuery) {
            sessionStorage.removeItem('concept_map_query');
            // Communicate back to parent
            window.parent.postMessage({type: 'streamlit_query', query: storedQuery}, '*');
        }
        </script>
        """
        st.components.v1.html(concept_search_js, height=0)

        # Quick search buttons organized by category (fallback for iframe communication)
...
- **Knowledge Base:** Prophix.Conso FC Knowledge Base
...
"""
        ...
        with btn_col4:
            ...
            if explanation_level != "Detailed":
                if st.button("Go Deeper", key="go_deeper", use_container_width=True, help="Get a more detailed explanation"):
                    st.session_state.query = query
                    st.session_state.go_deeper_query = query  # Store for detailed re-query
                    st.session_state.trigger_search = True
                    st.session_state.force_detailed = True
                    st.rerun()
            else:
                st.button("Already Detailed", disabled=True, use_container_width=True)
